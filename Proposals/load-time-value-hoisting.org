...
*Important*: If a client processes a form with side-effects, it needs to
keep a reference to this form or dump it immediately to a FASL.  Otherwise,
the form may be lost when its containing code is eliminated by some other
