...
** Changing the structure of LOAD-TIME-VALUE-INPUT
...
instructions and data.  Each client may want to process LOAD-TIME-VALUE
...
*Important*: If a client processes a LOAD-TIME-VALUE form with
side-effects, it needs to keep a reference to this form or dump it
immediately to a FASL.  Otherwise, the form may be lost when its containing
code is eliminated by some other compiler transformation.
...
wrapped in a THE-INSTRUCTION.  Cleavir could however provide a default
implementation similar to that of SBCL, using CONSTANTP. However, the exact
nature of this default implementation depends on the way FASLs are handled
by Cleavir.
...
applied to each reachable CONSTANT-INPUT and either turns it into an
...
** Summary
- Replace the %FORM slot of each LOAD-TIME-VALUE-AST by an %AST slot.
- Modify COMPILE-TOPLEVEL and COMPILE-AST to receive a client object as
  their last argument (or as their first argument?).
- Rewrite the primary method of COMPILE-AST on LOAD-TIME-VALUE ASTs.
- The hoisting of load-time-value forms remains an HIR transformation (or
  MIR?), but has to be modified to take nested LOAD-TIME-VALUE forms into
  account.  Constant-inputs are ignored at this step.
- A final pass, right before dumping a FASL, extracts and coalesces all
